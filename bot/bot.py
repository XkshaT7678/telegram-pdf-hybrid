...
from database import init_database, search_files, get_total_files, get_file_by_id
...
class PDFSearchBot:
    ...
    async def handle_web_deeplink(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle deep links from web search"""
        user_id = update.effective_user.id
        args = context.args
        
        if args and args[0].startswith('web_'):
            file_id = args[0][4:]  # Remove 'web_' prefix
            ...
            try:
                ...
                if not file_info:
                    await update.message.reply_text(
                        "❌ **File not found**\n\n"
                        "The requested file could not be found in our database. "
                        "Please search again from the website or bot.",
                ...
                await update.message.reply_text(
                    f"🌐 **Welcome from Web Search!**\n\n"
                    f"📚 **File:** `{file_info.file_name}`\n"
                    f"🔗 **Source:** Website Search\n\n"
                ...
                if verification_data.get('status') != 'success':
                    await update.message.reply_text(
                        f"❌ **Error generating verification link**\n\n"
                        f"Please try searching for the file again using the bot's search feature.",
                        parse_mode=ParseMode.MARKDOWN
                    )
                    return
                ...
                logger.error(f"Web deeplink error: {e}")
                await update.message.reply_text(
                    "❌ **Error processing web request**\n\n"
                    "Failed to process your web search request. "
                    "Please try searching for the file directly in this bot.",
                    parse_mode=ParseMode.MARKDOWN
                )
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if context.args and context.args[0].startswith('web_'):
            await self.handle_web_deeplink(update, context)
            return
